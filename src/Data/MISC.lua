...
-- Sets that give a raid difficulty as their description without being raid sets.
-- Blizzard reuses "Normal" for a handful of event and scenario rewards, and
-- nothing in the set data tells them apart from real tier.
-- Keyed by label rather than set ID: each of these exists once per armour type,
-- so the IDs differ per class while the label is shared by all of them.
-- The labels are localised, so this only takes effect on an English client.
addon.MiscSets.NON_RAID_LABELS = {
	["Darkmoon Faire"] = true,
	["Time's Keeper"] = true,
